...
##################### Follower Script ############################
# Last modified: 03/03/2025 # (11:46:50)
##################################################################
...
def get_global_position(vehicle):
    ...
    if msg:
        ...
        hdg = msg.hdg/100 #heading
        return [lat,lon,hdg,alt]
    ...
    while True:
        ...
        if current_pos:
            ...
            if dist_to_target is not None:
                ...
                if dist_to_target<= form_tolerance:
                    sleep(1)
                    break
...
def geo_distance_components(vehicle, lat2, lon2, tolerance, kp):
    ...
    if data is not None:
        ...
        if abs(x) > tolerance:
            send_velocity_setpoint(follower1,x*kp,y,0)
            sleep(0.1)

        if abs(y) > tolerance:
            send_velocity_setpoint(follower1,x,y*kp,0)
            time.sleep(0.1)
...
def change_formation(angle,distance,form_alt,tar_alt,min_pwm,max_pwm):
    ...
    sleep(5)
    formation(angle,distance,form_alt) #desired formation 
    ...
    st = time()
    counter = 0
    last_global_pos = [get_global_position(follower1)[0], get_global_position(follower1)[1], get_global_position(follower1)[2]] #lat, lon, hdg
    while True:
        ...
        if pos is not None:
            ...
            last_velocity[0] = pos.vx/local_scale_factor
            last_velocity[1] = pos.vy/local_scale_factor
            last_velocity[2] = pos.vz/local_scale_factor
        ...
        geo_distance_components(follower1,form_lat,form_lon, tolerance=2, kp=1.8)         
...
def main():
    ...
    while True:
        ...
        if rc_chan is not None:
            ...
            print("RC_channels not received from master.")
